...
from lib.common import set_signal_stop
from bin.daily_check import db, checker
...
class Consumer:
    ...
    def compare(self, chk_item):
        ...
        if std_pod:
            ...
            fmt_pod, res = checker.compare(chk_item["raw_pod"], chk_item["std_pod"])
            result = {
    ...
    def eval(self, chk_item):
        ...
        if rule:
            fmt_pod, res = checker.evaluate(pod, rule)
            result = {
                "fmt_pod": fmt_pod, "pod_result": res and "OK" or "NOK", "cod_state": CodState.DONE,
            }
            chk_item.update(result)
        else:
            func = checker.func(self.sys_info["sys_type"], chk_item["cmd"])
            if func:
    ...
    def format(self, text):
        fmt_pod = ""
        for i in text.splitlines(): fmt_pod += checker.format(i, check.Color.RED)
        return fmt_pod
...
if __name__ == "__main__":
    ...
    set_logger(f"daily_check_consumer_{args.sys_name}.log")
    try:
